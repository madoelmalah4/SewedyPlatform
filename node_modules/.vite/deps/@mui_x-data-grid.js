...
} from "./chunk-ZGK7XOT4.js";
import {
  useThemeProps
} from "./chunk-KGQEENOT.js";
import {
  createSelector,
  ...
  "node_modules/use-sync-external-store/cjs/use-sync-external-store-shim.development.js"(exports) {
    ...
    (function() {
      ...
      function useSyncExternalStore$2(subscribe, getSnapshot) {
        didWarnOld18Alpha || void 0 === React154.startTransition || (didWarnOld18Alpha = true, console.error(
      ...
      var React154 = require_react(), objectIs = "function" === typeof Object.is ? Object.is : is2, useState25 = React154.useState, useEffect43 = React154.useEffect, useLayoutEffect8 = React154.useLayoutEffect, useDebugValue = React154.useDebugValue, didWarnOld18Alpha = false, didWarnUncachedGetSnapshot = false, shim = "undefined" === typeof window || "undefined" === typeof window.document || "undefined" === typeof window.document.createElement ? useSyncExternalStore$1 : useSyncExternalStore$2;
      exports.useSyncExternalStore = void 0 !== React154.useSyncExternalStore ? React154.useSyncExternalStore : shim;
...
var useGridInitializeState = (initializer, privateApiRef, props) => {
  const isInitialized = React114.useRef(false);
...
var useGridClipboard = (apiRef, props) => {
  ...
  const handleCopy = React115.useCallback((event) => {
...
var useGridColumnMenu = (apiRef) => {
  ...
  const showColumnMenu = React116.useCallback((field) => {
  ...
  const hideColumnMenu = React116.useCallback(() => {
    const columnMenuState = gridColumnMenuSelector(apiRef.current.state);
  ...
  const toggleColumnMenu = React116.useCallback((field) => {
...
function useGridColumns(apiRef, props) {
  ...
  const previousColumnsProp = React117.useRef(props.columns);
  ...
  const setGridColumnsState = React117.useCallback((columnsState) => {
    var _a2, _b2;
  ...
  const getColumn = React117.useCallback((field) => gridColumnLookupSelector(apiRef)[field], [apiRef]);
  const getAllColumns = React117.useCallback(() => gridColumnDefinitionsSelector(apiRef), [apiRef]);
  const getVisibleColumns = React117.useCallback(() => gridVisibleColumnDefinitionsSelector(apiRef), [apiRef]);
  const getColumnIndex = React117.useCallback((field, useVisibleColumns = true) => {
    const columns = useVisibleColumns ? gridVisibleColumnDefinitionsSelector(apiRef) : gridColumnDefinitionsSelector(apiRef);
    return columns.findIndex((col) => col.field === field);
  }, [apiRef]);
  const getColumnPosition = React117.useCallback((field) => {
    const index = getColumnIndex(field);
    return gridColumnPositionsSelector(apiRef)[index];
  }, [apiRef, getColumnIndex]);
  const setColumnVisibilityModel = React117.useCallback((model) => {
    var _a2, _b2;
    const currentModel = gridColumnVisibilityModelSelector(apiRef);
  ...
  const updateColumns = React117.useCallback((columns) => {
    const columnsState = createColumnsState({
      apiRef,
  ...
  const setColumnVisibility = React117.useCallback((field, isVisible) => {
    const columnVisibilityModel = gridColumnVisibilityModelSelector(apiRef);
  ...
  const getColumnIndexRelativeToVisibleColumns = React117.useCallback((field) => {
    const allColumns = gridColumnFieldsSelector(apiRef);
    return allColumns.findIndex((col) => col === field);
  }, [apiRef]);
  const setColumnIndex = React117.useCallback((field, targetIndexPosition) => {
    const allColumns = gridColumnFieldsSelector(apiRef);
  ...
  const setColumnWidth = React117.useCallback((field, width) => {
  ...
  const preferencePanelPreProcessing = React117.useCallback((initialValue, value) => {
    var _a2;
  ...
  const addColumnMenuItems = React117.useCallback((columnMenuItems) => {
  ...
  const prevInnerWidth = React117.useRef(null);
  ...
  const hydrateColumns = React117.useCallback(() => {
  ...
  const isFirstRender = React117.useRef(true);
  React117.useEffect(() => {
    if (isFirstRender.current) {
      isFirstRender.current = false;
      return;
...
var useGridCsvExport = (apiRef, props) => {
  ...
  const getDataAsCsv = React119.useCallback((options = {}) => {
  ...
  const exportDataAsCsv = React119.useCallback((options) => {
...
var useGridPaginationModel = (apiRef, props) => {
  ...
  const densityFactor = useGridSelector(apiRef, gridDensityFactorSelector);
  const previousFilterModel = React120.useRef(gridFilterModelSelector(apiRef));
  ...
  const setPage = React120.useCallback((page) => {
  ...
  const setPageSize = React120.useCallback((pageSize) => {
    const currentModel = gridPaginationModelSelector(apiRef);
  ...
  const setPaginationModel = React120.useCallback((paginationModel) => {
    const currentModel = gridPaginationModelSelector(apiRef);
  ...
  const handleUpdateAutoPageSize = React120.useCallback(() => {
  ...
  const handleRowCountChange = React120.useCallback((newRowCount) => {
    if (newRowCount == null) {
      return;
    }
    const paginationModel = gridPaginationModelSelector(apiRef);
  ...
  const navigateToStart = React120.useCallback(() => {
    const paginationModel = gridPaginationModelSelector(apiRef);
    if (paginationModel.page !== 0) {
  ...
  const handleFilterModelChange = React120.useCallback((filterModel) => {
  ...
  React120.useEffect(handleUpdateAutoPageSize, [handleUpdateAutoPageSize]);
};
...
var useGridPrintExport = (apiRef, props) => {
  ...
  const doc = React121.useRef(null);
  const previousGridState = React121.useRef(null);
  const previousColumnVisibility = React121.useRef({});
  const previousRows = React121.useRef([]);
  const previousVirtualizationState = React121.useRef(null);
  React121.useEffect(() => {
    doc.current = ownerDocument(apiRef.current.rootElementRef.current);
  }, [apiRef, hasRootReference]);
  const updateGridColumnsForPrint = React121.useCallback((fields, allColumns, includeCheckboxes) => new Promise((resolve) => {
  ...
  const updateGridRowsForPrint = React121.useCallback((getRowsToExport) => {
  ...
  const handlePrintWindowLoad = React121.useCallback((printWindow, options) => {
    var _a, _b, _c, _d;
...
`;
  ...
  const handlePrintWindowAfterPrint = React121.useCallback((printWindow) => {
    var _a, _b;
  ...
  const exportDataAsPrint = React121.useCallback(async (options) => {
  ...
  const addExportMenuButtons = React121.useCallback((initialValue, options) => {
    var _a;
...
var useGridFilter = (apiRef, props) => {
  ...
  const updateFilteredRows = React122.useCallback(() => {
    apiRef.current.setState((state) => {
  ...
  const addColumnMenuItem = React122.useCallback((columnMenuItems, colDef) => {
    if (colDef == null || colDef.filterable === false || props.disableColumnFilter) {
      return columnMenuItems;
    }
    return [...columnMenuItems, "columnMenuFilterItem"];
  }, [props.disableColumnFilter]);
  const applyFilters = React122.useCallback(() => {
    updateFilteredRows();
    apiRef.current.forceUpdate();
  }, [apiRef, updateFilteredRows]);
  const upsertFilterItem = React122.useCallback((item) => {
  ...
  const upsertFilterItems = React122.useCallback((items) => {
    const filterModel = gridFilterModelSelector(apiRef);
  ...
  const deleteFilterItem = React122.useCallback((itemToDelete) => {
    const filterModel = gridFilterModelSelector(apiRef);
  ...
  const showFilterPanel = React122.useCallback((targetColumnField, panelId, labelId) => {
  ...
  const hideFilterPanel = React122.useCallback(() => {
    logger.debug("Hiding filter panel");
    apiRef.current.hidePreferences();
  }, [apiRef, logger]);
  const setFilterLogicOperator = React122.useCallback((logicOperator) => {
    const filterModel = gridFilterModelSelector(apiRef);
  ...
  const setQuickFilterValues = React122.useCallback((values) => {
    const filterModel = gridFilterModelSelector(apiRef);
  ...
  const setFilterModel = React122.useCallback((model, reason) => {
  ...
  const getFilterState = React122.useCallback((inputFilterModel) => {
  ...
  const preferencePanelPreProcessing = React122.useCallback((initialValue, value) => {
    var _a2;
  ...
  const flatFilteringMethod = React122.useCallback((params) => {
    var _a2;
  ...
  const handleColumnsChange = React122.useCallback(() => {
  ...
  const updateVisibleRowsLookupState = React122.useCallback(() => {
    apiRef.current.setState((state) => {
      return _extends({}, state, {
...
var useGridFocus = (apiRef, props) => {
  ...
  const lastClickedCell = React123.useRef(null);
  const hasRootReference = apiRef.current.rootElementRef.current !== null;
  const publishCellFocusOut = React123.useCallback((cell, event) => {
    if (cell) {
  ...
  const setCellFocus = React123.useCallback((id, field) => {
    const focusedCell = gridFocusCellSelector(apiRef);
  ...
  const setColumnHeaderFocus = React123.useCallback((field, event = {}) => {
  ...
  const setColumnHeaderFilterFocus = React123.useCallback((field, event = {}) => {
    const cell = gridFocusCellSelector(apiRef);
    publishCellFocusOut(cell, event);
    apiRef.current.setState((state) => {
  ...
  const setColumnGroupHeaderFocus = React123.useCallback((field, depth, event = {}) => {
    const cell = gridFocusCellSelector(apiRef);
    if (cell) {
      apiRef.current.publishEvent("cellFocusOut", apiRef.current.getCellParams(cell.id, cell.field), event);
    }
  ...
  const getColumnGroupHeaderFocus = React123.useCallback(() => gridFocusColumnGroupHeaderSelector(apiRef), [apiRef]);
  const moveFocusToRelativeCell = React123.useCallback((id, field, direction) => {
  ...
  const handleCellDoubleClick = React123.useCallback(({
    id,
    field
  }) => {
  ...
  const handleColumnHeaderFocus = React123.useCallback(({
    field
  }, event) => {
  ...
  const handleColumnGroupHeaderFocus = React123.useCallback(({
    fields,
    depth
  }, event) => {
    if (event.target !== event.currentTarget) {
  ...
  const handleBlur = React123.useCallback((_, event) => {
    var _a, _b;
  ...
  const handleCellMouseDown = React123.useCallback((params) => {
    lastClickedCell.current = params;
  }, []);
  const handleDocumentClick = React123.useCallback((event) => {
  ...
  const handleCellModeChange = React123.useCallback((params) => {
  ...
  const handleRowSet = React123.useCallback(() => {
    const cell = gridFocusCellSelector(apiRef);
...
var useGridKeyboardNavigation = (apiRef, props) => {
  ...
  const getCurrentPageRows = React124.useCallback(() => {
    return gridVisibleRowsWithPinnedRowsSelector(apiRef);
  }, [apiRef]);
  const headerFilteringEnabled = props.signature !== "DataGrid" && props.headerFilters;
  const goToCell = React124.useCallback((colIndex, rowId, closestColumnToUse = "left", rowSpanScanDirection = "up") => {
  ...
  const goToHeader = React124.useCallback((colIndex, event) => {
  ...
  const goToHeaderFilter = React124.useCallback((colIndex, event) => {
  ...
  const goToGroupHeader = React124.useCallback((colIndex, depth, event) => {
    logger.debug(`Navigating to header col ${colIndex}`);
    apiRef.current.scrollToIndexes({
      colIndex
    });
  ...
  const getRowIdFromIndex = React124.useCallback((rowIndex) => {
    var _a;
    return (_a = getCurrentPageRows()[rowIndex]) == null ? void 0 : _a.id;
  }, [getCurrentPageRows]);
  const handleColumnHeaderKeyDown = React124.useCallback((params, event) => {
  ...
  const handleHeaderFilterKeyDown = React124.useCallback((params, event) => {
  ...
  const handleColumnGroupHeaderKeyDown = React124.useCallback((params, event) => {
    const focusedColumnGroup = gridFocusColumnGroupHeaderSelector(apiRef);
  ...
  const checkIfCanStartEditing = React124.useCallback((initialValue, {
    event
  }) => {
    if (event.key === " ") {
      return false;
...
var useGridRowCount = (apiRef, props) => {
  ...
  const setRowCount = React125.useCallback((newRowCount) => {
  ...
  useGridRegisterPipeProcessor(apiRef, "exportState", stateExportPreProcessing);
  useGridRegisterPipeProcessor(apiRef, "restoreState", stateRestorePreProcessing);
  const handlePaginationModelChange = React125.useCallback((model) => {
...
var useGridPaginationMeta = (apiRef, props) => {
  ...
  const setPaginationMeta = React126.useCallback((newPaginationMeta) => {
...
var useGridPreferencesPanel = (apiRef, props) => {
  ...
  const hidePreferences = React127.useCallback(() => {
    apiRef.current.setState((state) => {
  ...
  const showPreferences = React127.useCallback((newValue, panelId, labelId) => {
...
var useGridCellEditing = (apiRef, props) => {
  const [cellModesModel, setCellModesModel] = React128.useState({});
  const cellModesModelRef = React128.useRef(cellModesModel);
  const prevCellModesModel = React128.useRef({});
  const {
    processRowUpdate,
  ...
  const throwIfNotInMode = React128.useCallback((id, field, mode) => {
  ...
  const handleCellEditStart = React128.useCallback((params) => {
    const {
      id,
      field,
      reason
  ...
  const handleCellEditStop = React128.useCallback((params) => {
    const {
      id,
      field,
      reason
  ...
  const getCellMode = React128.useCallback((id, field) => {
  ...
  const updateFieldInCellModesModel = React128.useCallback((id, field, newProps) => {
  ...
  const startCellEditMode = React128.useCallback((params) => {
    const {
      id,
      field
  ...
  const stopCellEditMode = React128.useCallback((params) => {
    const {
      id,
      field
  ...
  const setCellEditingEditCellValue = React128.useCallback(async (params) => {
    var _a, _b;
    const {
      id,
      field,
  ...
  const getRowWithUpdatedValuesFromCellEditing = React128.useCallback((id, field) => {
    const column = apiRef.current.getColumn(field);
    const editingState = gridEditRowsStateSelector(apiRef.current.state);
    const row = apiRef.current.getRow(id);
...
var useGridRowEditing = (apiRef, props) => {
  const [rowModesModel, setRowModesModel] = React129.useState({});
  const rowModesModelRef = React129.useRef(rowModesModel);
  const prevRowModesModel = React129.useRef({});
  const focusTimeout = React129.useRef(void 0);
  const nextFocusedCell = React129.useRef(null);
  const {
    processRowUpdate,
    onProcessRowUpdateError,
  ...
  const throwIfNotInMode = React129.useCallback((id, mode) => {
    if (apiRef.current.getRowMode(id) !== mode) {
      throw new Error(`MUI X: The row with id=${id} is not in ${mode} mode.`);
    }
  }, [apiRef]);
  const hasFieldsWithErrors = React129.useCallback((rowId) => {
    const editingState = gridEditRowsStateSelector(apiRef.current.state);
  ...
  const handleCellFocusIn = React129.useCallback((params) => {
  ...
  const handleRowEditStart = React129.useCallback((params) => {
    const {
      id,
      field,
      reason
  ...
  const handleRowEditStop = React129.useCallback((params) => {
    const {
      id,
      reason,
      field
  ...
  const getRowMode = React129.useCallback((id) => {
  ...
  const updateRowInRowModesModel = React129.useCallback((id, newProps) => {
  ...
  const updateOrDeleteRowState = React129.useCallback((id, newProps) => {
    apiRef.current.setState((state) => {
      const newEditingState = _extends({}, state.editRows);
      if (newProps !== null) {
  ...
  const startRowEditMode = React129.useCallback((params) => {
    const {
      id
  ...
  const stopRowEditMode = React129.useCallback((params) => {
    const {
      id
  ...
  const setRowEditingEditCellValue = React129.useCallback((params) => {
    const {
      id,
      field,
      value,
  ...
  const getRowWithUpdatedValuesFromRowEditing = React129.useCallback((id) => {
    const editingState = gridEditRowsStateSelector(apiRef.current.state);
    const row = apiRef.current.getRow(id);
    if (!editingState[id]) {
      return apiRef.current.getRow(id);
...
var useGridEditing = (apiRef, props) => {
  ...
  const debounceMap = React130.useRef({});
  const {
    isCellEditable: isCellEditableProp
  } = props;
  const isCellEditable = React130.useCallback((params) => {
  ...
  const runPendingEditCellValueMutation = React130.useCallback((id, field) => {
    if (!debounceMap.current[id]) {
      return;
    }
    if (!field) {
  ...
  const setEditCellValue = React130.useCallback((params) => {
    const {
      id,
      field,
      debounceMs
  ...
  const getRowWithUpdatedValues = React130.useCallback((id, field) => {
    return props.editMode === GridEditModes.Cell ? apiRef.current.getRowWithUpdatedValuesFromCellEditing(id, field) : apiRef.current.getRowWithUpdatedValuesFromRowEditing(id);
  }, [apiRef, props.editMode]);
  const getEditCellMeta = React130.useCallback((id, field) => {
    var _a;
    const editingState = gridEditRowsStateSelector(apiRef.current.state);
...
var useGridRows = (apiRef, props) => {
  ...
  const lastUpdateMs = React131.useRef(Date.now());
  const lastRowCount = React131.useRef(props.rowCount);
  const timeout = useTimeout();
  const getRow = React131.useCallback((id) => {
  ...
  const getRowId = React131.useCallback((row) => {
  ...
  const throttledRowsChange = React131.useCallback(({
    cache: cache2,
    throttle: throttle2
  }) => {
    const run = () => {
  ...
  const setRows = React131.useCallback((rows) => {
  ...
  const updateRows = React131.useCallback((updates) => {
  ...
  const updateServerRows = React131.useCallback((updates, groupKeys) => {
    const nonPinnedRowsUpdates = computeRowsUpdates(apiRef, updates, props.getRowId);
    const cache2 = updateCacheWithNewRows({
      updates: nonPinnedRowsUpdates,
      getRowId: props.getRowId,
  ...
  const setLoading = React131.useCallback((loading) => {
  ...
  const getRowModels = React131.useCallback(() => {
    const dataRows = gridDataRowIdsSelector(apiRef);
    const idRowsLookup = gridRowsLookupSelector(apiRef);
    return new Map(dataRows.map((id) => [id, idRowsLookup[id] ?? {}]));
  }, [apiRef]);
  const getRowsCount = React131.useCallback(() => gridRowCountSelector(apiRef), [apiRef]);
  const getAllRowIds = React131.useCallback(() => gridDataRowIdsSelector(apiRef), [apiRef]);
  const getRowIndexRelativeToVisibleRows = React131.useCallback((id) => {
    const row = apiRef.current.getRow(id);
    const {
      rowToIndexMap
  ...
  const setRowChildrenExpansion = React131.useCallback((id, isExpanded) => {
  ...
  const getRowNode = React131.useCallback((id) => gridRowTreeSelector(apiRef)[id] ?? null, [apiRef]);
  const getRowGroupChildren = React131.useCallback(({
  ...
  const setRowIndex = React131.useCallback((rowId, targetIndex) => {
  ...
  const replaceRows = React131.useCallback((firstRowToRender, newRows) => {
  ...
  const groupRows = React131.useCallback(() => {
  ...
  const handleStrategyActivityChange = React131.useCallback(() => {
  ...
  const applyHydrateRowsProcessor = React131.useCallback(() => {
    apiRef.current.setState((state) => {
...
function useGridParamsApi(apiRef, props) {
  var _a;
  const getColumnHeaderParams = React132.useCallback((field) => ({
    field,
    colDef: apiRef.current.getColumn(field)
  }), [apiRef]);
  const getRowParams = React132.useCallback((id) => {
    const row = apiRef.current.getRow(id);
    if (!row) {
  ...
  const getCellParamsForRow = React132.useCallback((id, field, row, {
    cellMode,
    colDef,
    hasFocus,
    rowNode,
  ...
  const getCellParams = React132.useCallback((id, field) => {
    var _a2;
    const row = apiRef.current.getRow(id);
    const rowNode = apiRef.current.getRowNode(id);
    if (!row || !rowNode) {
  ...
  const getCellValue2 = React132.useCallback((id, field) => {
    const colDef = apiRef.current.getColumn(field);
    const row = apiRef.current.getRow(id);
    if (!row) {
      throw new MissingRowIdError(`No row with id #${id} found`);
  ...
  const getRowValue = React132.useCallback((row, colDef) => {
  ...
  const getRowFormattedValue = React132.useCallback((row, colDef) => {
  ...
  const getColumnHeaderElement = React132.useCallback((field) => {
    if (!apiRef.current.rootElementRef.current) {
      return null;
    }
    return getGridColumnHeaderElement(apiRef.current.rootElementRef.current, field);
  }, [apiRef]);
  const getRowElement = React132.useCallback((id) => {
    if (!apiRef.current.rootElementRef.current) {
      return null;
    }
    return getGridRowElement(apiRef.current.rootElementRef.current, id);
  }, [apiRef]);
  const getCellElement = React132.useCallback((id, field) => {
    if (!apiRef.current.rootElementRef.current) {
      return null;
    }
...
function useGridDimensions(apiRef, props) {
  ...
  const previousSize = React137.useRef(void 0);
  const getRootDimensions = React137.useCallback(() => gridDimensionsSelector(apiRef.current.state), [apiRef]);
  const setDimensions = React137.useCallback((dimensions) => {
    apiRef.current.setState((state) => _extends({}, state, {
      dimensions
    }));
    if (apiRef.current.rootElementRef.current) {
  ...
  const debouncedUpdateDimensions = React137.useMemo(() => props.resizeThrottleMs > 0 ? throttle(() => {
    updateDimensionCallback();
    apiRef.current.publishEvent("debouncedResize", rootDimensionsRef.current);
  }, props.resizeThrottleMs) : void 0, [apiRef, props.resizeThrottleMs, updateDimensionCallback]);
  React137.useEffect(() => debouncedUpdateDimensions == null ? void 0 : debouncedUpdateDimensions.clear, [debouncedUpdateDimensions]);
  const apiPublic = {
    resize,
    getRootDimensions
  };
  ...
  const handleRootMount = React137.useCallback((root) => {
...
function useColumnVirtualizationDisabled(apiRef) {
  const promise = React142.useRef(void 0);
...
var useGridColumnResize = (apiRef, props) => {
  ...
  const initialOffsetToSeparator = React142.useRef(null);
  const resizeDirection = React142.useRef(null);
  const stopResizeEventTimeout = useTimeout();
  const touchId = React142.useRef(void 0);
...
var useGridRowSpanning = (apiRef, props) => {
  ...
  const updateRowSpanningState = React143.useCallback((renderContext, resetState = false) => {
    const {
      range: range2,
      rows: visibleRows
    } = getVisibleRows(apiRef, {
  ...
  const resetRowSpanningState = React143.useCallback(() => {
    const renderContext = gridRenderContextSelector(apiRef);
...
function GridColumnGroupHeader(props) {
  ...
  const style = React145.useMemo(() => attachPinnedStyle(_extends({}, props.style), isRtl, pinnedPosition, pinnedOffset), [pinnedPosition, pinnedOffset, props.style, isRtl]);
